# My thought with how to deal with different experimental structures/data types etc. is:
# we assume that everyone has ‘subject/session_xx/??????%02d???.file’  or ‘subject/??????%02d???.file’, the user can specify a regex for the run in opt (we put a sensible default), and if their structure is really different they need to define their own convert_to_mne and load_eeglab…. 
# No matter what the input is, we output into a standardised format which is formatted: #‘subject/subject_session%02d/subject_session%02d_run%02d.mne_extension’
